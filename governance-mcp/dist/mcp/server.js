...
function createAjv() {
    ...
    const AjvCtor = AjvNS.default ?? AjvNS;
    const addFormatsFn = AjvFormatsNS.default ?? AjvFormatsNS;
...
export function createGovernanceMcpServer() {
    const server = new Server({ name: "mindtrace-governance-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (name === "contracts.validate") {
                const schemaFile = String(args?.schemaFile || "");
                const payload = args?.payload;
                if (!schemaFile)
                    throw new Error("Missing required argument: schemaFile");
