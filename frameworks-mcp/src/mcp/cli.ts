#!/usr/bin/env node
// frameworks-mcp/src/mcp/cli.ts

import { startFrameworksMcpStdioServer } from "./server.js";

function printHelp() {
  console.log(`Usage: mindtrace-frameworks-mcp [options]

MindTrace Frameworks MCP server (stdio)
...
async function main() {
  ...
  await startFrameworksMcpStdioServer();
}

main().catch((error) => {
  console.error("Frameworks MCP server error:", error);
  process.exit(1);
});
